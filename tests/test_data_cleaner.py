...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        # No deben quedar valores faltantes en las columnas filtradas
        self.assertEqual(int(result["name"].isna().sum()), 0)
        self.assertEqual(int(result["age"].isna().sum()), 0)

        # El DataFrame resultante debe tener estrictamente menos filas que el original
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # 1) El DataFrame original NO debe haber sido modificado: mantiene los espacios
        self.assertEqual(df["name"].iloc[0], " Alice ")
        self.assertEqual(df["name"].iloc[3], " Carol  ")

        # 2) En el resultado, los valores de "name" están sin espacios al inicio/final
        ...
        # 3) Las columnas no especificadas ("city") permanecen idénticas
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        # "age" es numérica -> debe rechazarse con TypeError
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df = pd.DataFrame({
            ...
            "age": [25, 28, 30, 32, 35, 120],  # 120 es el outlier
        ...
        # El valor extremo (120) debe haber sido eliminado del resultado
        self.assertNotIn(120, result["age"].values)

        # Al menos uno de los valores no extremos debe seguir presente
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        # "city" es una columna de texto -> debe rechazarse con TypeError
        with self.assertRaises(TypeError):
